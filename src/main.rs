...
use std::path;
mod board;
use board::{init_board, Piece, PieceColor, PieceType, Tile};
...
impl event::EventHandler<ggez::GameError> for MainState {
    ...
    fn mouse_button_up_event(
        ...
        match btn {
            MouseButton::Left => {
                if let HoldingPiece::True(..) = self.holding {
...
fn draw_piece(
    ...
    let draw_param = if !is_mouse_coords {
        DrawParam::default().dest([x + OFFSET_X, y + OFFSET_Y])
    } else {
        DrawParam::default().dest([x - 35.0, y - 35.0])
    };
    match piece {
        Piece {
...
fn check_if_can_place_piece(x: f32, y: f32, state: &mut MainState) {
    ...
    if x >= 115.0 && x <= 885.0 && y >= 105.0 && y <= 885.0 {
        ...
        if x % 1.0 >= 0.15 && x % 1.0 <= 0.8 && y % 1.0 >= 0.1 && y % 1.0 <= 0.85 {
            ...
            let y = y.floor() as usize - 1;
            let pos2 = (x, y);
            check_if_valid_move(pos1, pos2, &mut state.chessboard);
        }
    }
    state.holding = HoldingPiece::False;
}
fn check_if_valid_move(pos1: (usize, usize), pos2: (usize, usize), chessboard: &mut [[Tile; 8]; 8]) {
    let (col1, row1) = pos1;
    let (col2, row2) = pos2;
    println!(
        "col1: {}, row1: {}, col2: {}, row2: {}",
    ...
    let tile1 = chessboard[col1][row1];
    let tile2 = chessboard[col2][row2];
    ...
    chessboard[col2][row2] = tile1;
    chessboard[col1][row1] = Tile::Nothing;
}
